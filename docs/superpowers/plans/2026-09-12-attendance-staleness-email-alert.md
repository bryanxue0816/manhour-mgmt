...
8. **spec 仲裁与勘误（写计划时已裁决，执行时照此；其中 🟠 项需用户确认）**：
   ...
   - **终审后挂账（产品确认，不在 walking skeleton 内改码）**：L1——首封邮件正文（alert-template.ts STALE_CONVENTION，spec §5.4 冻结原文）与子页 CONVENTION_NOTE 措辞「超过 3 个自然日即触发」与代码阈值 `>= 3`（首发邮件 daysSince 恰为 3）字面矛盾，候选「达到 3 个自然日即触发」，待产品确认后 spec+模板+模板测试同改；F2——周五末次成功、周一文件晚于 09:20 落地时，09:20 首发停摆警、15:20 发恢复通知，健康管线周一同日双发，阈值=3 本身已裁决，「同日双发」是否接受待产品确认（不接受再议扫描时刻或边界，不改本期代码）。
