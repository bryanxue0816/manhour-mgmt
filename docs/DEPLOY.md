...
| `exit=1`，日志 `alert-email-failed` | 配置通过但 SMTP 连接/认证失败（中继不可达、口令错），页面频控卡显示最近错误。按邮件类型区分（Erratum T）：**首警/重复警**同日不重试，下个扫描时刻按频控再试；**恢复通知发送失败不补发**——恢复意图落盘时状态已转为正常，后续扫描恒为 skip，只能人工看容器日志与页面频控卡的最近错误。 |
